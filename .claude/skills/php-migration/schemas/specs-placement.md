...
**用途:**
- `api-catalog-builder` が `artifacts/api-catalog.yaml` を生成する際の型定義・エンドポイント仕様として参照
...
**用途:**
- `domain-modeler` が `artifacts/domain-model.yaml` のエンティティ定義・リレーション・フィールド型を確定する際に参照
