...
`php-migration/docs/prompt-templates/phase-c-controller-migrator.md` を読み込み実行します。

#### template-migrator

`php-migration/docs/prompt-templates/phase-c-template-migrator.md` を読み込み実行します。

#### service-builder

`php-migration/docs/prompt-templates/phase-c-service-builder.md` を読み込み実行します。

#### api-client-builder

`php-migration/docs/prompt-templates/phase-c-api-client-builder.md` を読み込み実行します。

いずれかが `NEEDS_RULE` フラグを出力した場合:
- `php-migration/docs/failure-handling.md` の NEEDS_RULE 振る舞いフローに従って処理します
- `php-migration/docs/prompt-templates/phase-b-mapping-rule-author.md` を appendモードで実行し、ルールを追記します
...
`php-migration/docs/prompt-templates/phase-d-dod-checker.md` を読み込み実行します。
結果を `php-migration/dod-results/$ARGUMENTS.yaml` に出力します。

#### snapshot-comparator

`php-migration/docs/prompt-templates/phase-d-snapshot-comparator.md` を読み込み実行します。
結果を `php-migration/dod-results/$ARGUMENTS-snapshot.yaml` に出力します。

#### code-reviewer

`php-migration/docs/prompt-templates/phase-d-code-reviewer.md` を読み込み実行します。
結果を `php-migration/dod-results/$ARGUMENTS-codereview.yaml` に出力します。

#### test-generator

`php-migration/docs/prompt-templates/phase-d-test-generator.md` を読み込み実行します。

### 4. 結果の統合と報告

`dod-checker` の `overall` が `FAIL` の場合:
- `php-migration/docs/failure-handling.md` の REVIEW_REQUIRED フローに従います
